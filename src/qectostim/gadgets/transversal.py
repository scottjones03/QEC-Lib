...
class TransversalGate(Gadget, TransversalGadgetMixin):
    ...
    def emit_next_phase(
        ...
        ctx : DetectorContext
            Detector context (not modified here).
    ...
    def build_schedule(
        ...
        """Build schedule for transversal gate."""
            ...
            for _ in range(self.num_rounds_before):
                for block_name, block_info in layout.blocks.items():
                    scheduler.schedule_stabilizer_round(
                        ...
                        data_offset=block_info.data_qubit_range[0],
                        ancilla_offset=block_info.ancilla_qubit_range[0],
                        time=time,
                    )
                time += 1.0
        
        # The transversal gate layer
        if self.is_two_qubit:
            # Two-qubit gate between corresponding qubits
            block0 = layout.blocks["block_0"]
            block1 = layout.blocks["block_1"]
            n = codes[0].n
            
            gate_pairs = []
            for i in range(n):
                ctrl = block0.data_qubit_range[0] + i
                tgt = block1.data_qubit_range[0] + i
                gate_pairs.append((ctrl, tgt))
            
            scheduler.schedule_transversal_gate(
                gate_name=self.stim_gate,
                qubit_pairs=gate_pairs,
                time=time,
            )
        else:
            # Single-qubit gate on all data qubits
            block0 = layout.blocks["block_0"]
            n = codes[0].n
            
            qubits = [
                (block0.data_qubit_range[0] + i,)
                for i in range(n)
            ]
