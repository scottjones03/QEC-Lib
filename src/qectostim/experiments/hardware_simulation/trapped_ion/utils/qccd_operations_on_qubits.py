...
class OneQubitGate(QubitOperation):
    ...
    GATE_DURATION = DEFAULT_CALIBRATION.single_qubit_gate_time
    ...
    def calculateFidelity(self) -> None:
        ...
        self.calculateOperationTime()
        chain_length = len(self._trap.ions)
        motional_quanta = self._trap.motionalMode
        self._fidelity = DEFAULT_FIDELITY_MODEL.single_qubit_gate_fidelity(
            chain_length, motional_quanta
        )
...
class Measurement(QubitOperation):
    ...
    # Constants now sourced from physics.py DEFAULT_CALIBRATION
    MEASUREMENT_TIME = DEFAULT_CALIBRATION.measurement_time
    ...
    def calculateFidelity(self) -> None:
        self._fidelity = DEFAULT_FIDELITY_MODEL.measurement_fidelity()
...
class QubitReset(QubitOperation):
    ...
    # Constants now sourced from physics.py DEFAULT_CALIBRATION
    RESET_TIME = DEFAULT_CALIBRATION.reset_time
    ...
    def calculateFidelity(self) -> None:
        self._fidelity = DEFAULT_FIDELITY_MODEL.reset_fidelity()
...
class TwoQubitMSGate(QubitOperation):
    ...
    # Constants now sourced from physics.py DEFAULT_CALIBRATION
    OP_TIME = DEFAULT_CALIBRATION.ms_gate_time
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Drop legacy use_chain_fidelity_model kwarg silently
        kwargs.pop("use_chain_fidelity_model", None)
        super().__init__(
    ...
    def calculateFidelity(self) -> None:
        ...
        chain_length = len(self._trap.ions)
        motional_quanta = self._trap.motionalMode
        self._fidelity = DEFAULT_FIDELITY_MODEL.ms_gate_fidelity(
            chain_length, motional_quanta
        )
    ...
    def __eq__(self, other: object) -> bool:
        if not isinstance(other, TwoQubitMSGate):
            return NotImplemented
        return frozenset(self.ionsActedIdxs) == frozenset(other.ionsActedIdxs)

    def __hash__(self) -> int:
        return hash(frozenset(self.ionsActedIdxs))
