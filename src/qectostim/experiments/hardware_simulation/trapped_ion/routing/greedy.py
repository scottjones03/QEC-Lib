# src/qectostim/experiments/hardware_simulation/trapped_ion/routing/greedy.py
"""
Junction-based QCCD ion routing (greedy algorithm).
...
def route_ions_junction(
    ...
    while remaining:
        ...
        for a_idx, req in sorted_moves:
            ...
            try:
                all_paths = list(nx.all_shortest_paths(nxg, src_id, dst_id))
            ...
            # Reserve — directly mutate node ion counts to prevent
            # subsequent paths from overcommitting the same node.
            # Faithful port of old code's in-place numIons += 1 pattern.
        ...
        for a_idx, (req, path_nodes, dest_trap) in movements.items():
            if dest_trap.num_ions >= trap_capacity:
                ...
                # Decrement dest count since the ion will be sent back
                # after the gate executes (faithful port of old code's
                # destTrap.numIons -= 1).
                dest_trap.num_ions -= 1
            else:
