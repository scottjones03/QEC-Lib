...
def ionRouting(
    qccdArch,  # QCCDArchitecture, WISEArchitecture, or QCCDArch (needs .graph, .ions, .nodes, .crossingEdges, .refreshGraph())
    operations: Sequence[QubitGate],
    trapCapacity: int
) -> Tuple[Sequence[Operation], Sequence[int]]:
    twoqubitGatesForAncillaIons: Dict[int, List[MSGate]] = {}
    for op in operations:
        if isinstance(op, MSGate):
            ion1, ion2 = op.ions
            ancilla, data = sorted(
    ...
    toMoveCandidates: Dict[int, MSGate] = {}
    ...
    while operationsLeft:
        ...
        movements: Dict[int, Tuple[MSGate, List[QCCDNode], Trap]] = {}
        ...
        toForward: Dict[MSGate, Tuple[int, List[QCCDNode], Trap, Optional[Trap]]] = {}
        ...
        while toForward:
            ...
            for op, (ancillaIdx, qccdNodes, goBackTrap) in orderedToForward:
                ...
                # n1Idx = parent trap/node idx (for qccdNodes pairing)
                # n1_ion = ion idx (for graph path lookup)
                ancilla_ion = qccdArch.ions[ancillaIdx]
                n1_ion = ancilla_ion.idx
                n1Idx = ancilla_ion.parent.idx
                trap = qccdNodes[-1]
                srcTrap = goBackTrap
                ...
                forwardingPath = nx.shortest_path(qccdArch.graph, n1_ion, n2Idx)
                ms: List[Operation] = []
                for fn1, fn2 in zip(forwardingPath[:-1], forwardingPath[1:]):
                    ms.extend(qccdArch.graph.edges[fn1, fn2]["operations"])
                    
                for m in ms:
                    if isinstance(m, CrystalRotation):
                        ionsInvolvedNow.extend(m.ions)
                    # Hack FIXME
                    if isinstance(m, GateSwap) and m._ions[0] == m._ions[1]:
                        continue
                    m.run()
                    allOps.append(m)
