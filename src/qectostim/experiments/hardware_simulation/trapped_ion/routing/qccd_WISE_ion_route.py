...
from ..architecture import (
    Ion, QubitIon, CoolingIon, SpectatorIon,
    QCCDNode, Trap, ManipulationTrap, StorageTrap, Junction,
    Crossing, Operations, QCCDComponent,
)
# Import qubit operations from consolidated operations module
from ..operations import (
    Operation,
    QubitGate, SingleQubitGate, Measurement, MSGate, GateSwap,
    ParallelOperation,
)
...
def _patch_and_route(
    ...
    wiseArch,  # WISEArchitecture or QCCDWiseArch (needs .m, .n, .k)
    ...
    while total_remaining>0:
        ...
        for tiling_idx, (off_r, off_c) in enumerate(tilings):
            ...
            for region in patch_regions:
                ...
                try:
                    patch_layouts, patch_schedule, prev_pmax = ReconfigurationPlanner._optimal_QMR_for_WISE(
                        patch_grid,
                        patch_pairs,
                        k=wiseArch.k,
                        BT=BT_patch,
...
def _rebuild_schedule_for_layout(
    ...
    for cycle_idx in range(max_cycles):
        ...
        for region in patch_regions:
            ...
            try:
                patch_layouts, patch_schedule, _ = ReconfigurationPlanner._optimal_QMR_for_WISE(
                    patch_grid,
...
def _apply_layout_as_reconfiguration(
    arch,  # WISEArchitecture or QCCDArch (needs .ions, ._manipulationTraps, .refreshGraph())
    wiseArch,  # WISEArchitecture or QCCDWiseArch (needs .m, .n, .k)
    oldArrangementArr: np.ndarray,
    ...
    the slice solver) and:
      ...
      - build a ReconfigurationPlanner.physicalOperation that moves ions from
    ...
    reconfig = ReconfigurationPlanner.physicalOperation(
        newArrangement, wiseArch, oldArrangementArr, newArrangementArr, schedule=schedule, initial_placement=initial_placement
    )

    # Store per-ion movements for animation: (ion_idx, src_zone, tgt_zone)
    _n_rows, _n_cols = oldArrangementArr.shape
    _old_pos: Dict[int, Tuple[int, int]] = {}
    _new_pos: Dict[int, Tuple[int, int]] = {}
    for _d in range(_n_rows):
        for _c in range(_n_cols):
            _old_pos[int(oldArrangementArr[_d][_c])] = (_d, _c)
            _new_pos[int(newArrangementArr[_d][_c])] = (_d, _c)
    movements = []
    for _ion_idx, (_od, _oc) in _old_pos.items():
        _nd, _nc = _new_pos.get(_ion_idx, (_od, _oc))
        if (_od, _oc) != (_nd, _nc):
            movements.append((_ion_idx, f"trap_{_od}_{_oc}", f"trap_{_nd}_{_nc}"))
    reconfig._ion_movements = movements
...
def ionRoutingWISEArch(
    arch,  # WISEArchitecture (has .m/.n/.k/.ions/.refreshGraph) or QCCDArch (legacy)
    operations: Sequence[QubitGate],
    lookahead: int = 2,
    subgridsize: Tuple[int, int, int] = (6, 4, 1), 
    base_pmax_in: int = None,
    toMoveOps: Sequence[Sequence[MSGate]]= None,
    wise_config=None,  # Optional: QCCDWiseArch for legacy callers that pass QCCDArch as arch
    round_repetition_hint: Optional[Dict[str, Any]] = None,
    ...
    # Resolve WISE grid dimensions (m, n, k).
    # If wise_config is provided (legacy callers passing QCCDArch + QCCDWiseArch),
    # use it.  Otherwise arch itself must have .m, .n, .k (WISEArchitecture).
    if wise_config is not None:
        wiseArch = wise_config
    elif hasattr(arch, 'm') and hasattr(arch, 'n') and hasattr(arch, 'k'):
        wiseArch = arch
    else:
        raise TypeError(
            "ionRoutingWISEArch: arch must have .m/.n/.k attributes "
            "(WISEArchitecture), or provide wise_config (QCCDWiseArch)."
        )
    ...
    # schedule=None so ReconfigurationPlanner can treat them as repeats.
    ...
    # --- QECMetadata-driven cache hints ---
    _hint_ion_return: bool = False
    _phases_force_recheck: set = set()
    _cached_start_layout: Optional[np.ndarray] = None
    if round_repetition_hint is not None:
        _hint_ion_return = round_repetition_hint.get("ion_return", False)
        for ms_round_idx in round_repetition_hint.get("force_recheck_at", []):
            _phases_force_recheck.add(ms_round_idx)
        logger.info(
            "%s round_repetition_hint: ion_return=%s, force_recheck_at=%s",
            PATCH_LOG_PREFIX,
            _hint_ion_return,
            _phases_force_recheck,
        )

    # ------------------------------------------------------------------
    # 4) Execute operations, routing between parallel MS rounds as needed
    # ------------------------------------------------------------------
    _routing_t0 = time.time()
    _total_ms_rounds = len(toMoves)
    while operationsLeft:
        # -- Emit progress --
        if progress_callback is not None:
            from .config import RoutingProgress
            progress_callback(RoutingProgress(
                stage="routing",
                current=idx,
                total=_total_ms_rounds,
                gates_remaining=len(operationsLeft),
                elapsed_seconds=time.time() - _routing_t0,
                message=f"Round {idx}/{_total_ms_rounds}",
                extra={"reconfig_time": reconfigTime},
            ))
        logger.info(
        ...
        if len(toMoves) > idx:
            if (not hadMultipleTilingSteps) and tiling_steps and tiling_steps[0]:
                ...
                # Ion-return layout shortcut: if _hint_ion_return is
                # True and the current layout matches the cached start
                # layout, we know the ions have returned to their
                # starting positions.  Any block_cache entry for this
                # key can be replayed with the *full* schedule (not
                # just layout-only), because the ion arrangement is
                # identical to when the cache entry was first built.
                if (
                    _hint_ion_return
                    and _cached_start_layout is not None
                    and np.array_equal(oldArrangementArr, _cached_start_layout)
                ):
                    recheck_cache = True
                    logger.info(
                        "%s idx=%d: ion-return layout matches cached start "
                        "layout — forcing cache recheck for full replay",
                ...
                if tiling_steps_from_cache:
                    ...
                    # When ion-return is confirmed (layout matches start),
                    # the cached schedule is directly applicable — no need
                    # to null-out the first reconfiguration's schedule.
                    if (
                        _hint_ion_return
                        and _cached_start_layout is not None
                        and np.array_equal(oldArrangementArr, _cached_start_layout)
            ...
            if tiling_steps_from_cache and pending_first_cached_reconfig:
                ...
                # Capture starting layout for ion-return cache hint
                if _hint_ion_return and _cached_start_layout is None:
                    _cached_start_layout = oldArrangementArr.copy()
                logger.info(
        ...
        # Force recheck at phase boundaries identified by QECMetadata
        if idx in _phases_force_recheck:
            recheck_cache = True
            logger.info(
                "%s idx=%d: forcing cache recheck at phase boundary",
                PATCH_LOG_PREFIX, idx,
            )
        if blk_idx == blk_end:
            cache_for_block = {}
