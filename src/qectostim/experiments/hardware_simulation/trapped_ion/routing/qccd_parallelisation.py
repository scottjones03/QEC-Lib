...
# Import physics constants from single source of truth
...
def calculateDephasingFidelity(time: float) -> None:
    ...
    # Dephasing noise: https://journals.aps.org/pra/pdf/10.1103/PhysRevA.99.022330
    T2 = _CAL.t2_time  # seconds (from physics.py)
    return 1 - (1 - np.exp(-time / T2)) / 2
...
def calculateDephasingFromIdling(
    ...
    """
    Compute dephasing per ion, based on idling intervals between QubitGate ops.
    ...
    #    ion_idling_operations[ion] = QubitGate that ends each idle interval
    ion_idling_operations: Dict[Ion, List[QubitGate]] = {ion: [] for ion in all_ions}

    # Set of ions currently idle
    idling_ions: Set[Ion] = set(all_ions)

    # 5) Sweep through the timeline, updating idling intervals on QubitGate start/end
    for t, op, kind in events:
        # Only QubitGate ops determine "idle" vs "active" for dephasing
        if not isinstance(op, QubitGate):
            continue
    ...
    for ion, idling_times in ion_idling_times.items():
        # Number of completed idle intervals = number of recorded end QubitGate ops
