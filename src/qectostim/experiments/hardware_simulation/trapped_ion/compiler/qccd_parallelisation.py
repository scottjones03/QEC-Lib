...
def happensBeforeForOperations(
    ...
    # Step 1b: Add tick-epoch-based edges for QubitOperations.
    # If two QubitOperations share an ion AND belong to different tick
    # epochs (from the stim circuit's TICK boundaries), enforce that the
    # earlier epoch completes first.  This preserves the stim circuit's
    # happens-before ordering without the coarseness of global TICK
    # barriers (operations on *different* ions in different epochs can
    # still be parallelised).
    def _program_order_key(op: Operation) -> tuple:
        """Sort key reflecting the original stim program order."""
        return (getattr(op, '_tick_epoch', -1), getattr(op, '_stim_origin', -1))

    _qubit_ops_by_ion: Dict[int, List[Operation]] = defaultdict(list)
    for op in operationSequence:
        if not hasattr(op, '_tick_epoch') or getattr(op, '_tick_epoch', -1) < 0:
            continue
        for ion in getattr(op, 'ions', ()):
            _qubit_ops_by_ion[ion.idx].append(op)

    for _ion_idx, _ops in _qubit_ops_by_ion.items():
        # Sort by program order so edges point in the right direction
        _ops.sort(key=_program_order_key)
        for i in range(len(_ops)):
            epoch_i = _ops[i]._tick_epoch
            for j in range(i + 1, len(_ops)):
                epoch_j = _ops[j]._tick_epoch
                if epoch_j > epoch_i:
                    _add_edge(_ops[i], _ops[j])
                    # Only need edge to the first op in a later epoch
                    # (transitivity handles the rest within that epoch)
                    break
