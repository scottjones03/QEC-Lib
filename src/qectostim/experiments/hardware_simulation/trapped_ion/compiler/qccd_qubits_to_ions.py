...
def _partitionClusterIons(
    ...
    # Track (coord, original_index) pairs to avoid collisions when ions share coordinates
    tagged = [(list(c), i) for i, c in enumerate(coords)]
    partitions = [tagged]
    splitAxisIsX = True
    ...
    while max([len(p) for p in partitions])>trapCapacity:
        ...
        if iteration > max_iterations:
            raise RuntimeError(
                f"_partitionClusterIons exceeded {max_iterations} iterations. "
                f"Cannot partition {len(coords)} coords into clusters of size {trapCapacity}."
            )
        toSplit = [p for p in partitions if len(p)>trapCapacity]
        made_progress = False
        for p in toSplit:
            splitAxisVals = [float(c[int(splitAxisIsX)]) for c, _ in p]
            # Use median for better splitting when values cluster
            medAxisVal = float(np.median(splitAxisVals))
            p1 = []
            p2 = []
            for item, splAxisVal in zip(p, splitAxisVals):
                if splAxisVal <= medAxisVal:
                    p1.append(item)
                else:
                    p2.append(item)
            
            # Handle edge case: if split didn't work (all same value), force split
            if not p1 or not p2:
                # Force a split by index
                mid = len(p) // 2
                p1 = p[:mid]
                p2 = p[mid:]
            
            if p1 and p2 and (len(p1) < len(p) or len(p2) < len(p)):
                made_progress = True
        ...
        if not made_progress and toSplit:
            # Try the other axis
            pass
    ...
    clusters = []
    for p in partitions:
        clusterIons = [ions[orig_idx] for _, orig_idx in p]
        clusterCoords = [c for c, _ in p]
        clusterCentre = np.mean(clusterCoords, axis=0)
