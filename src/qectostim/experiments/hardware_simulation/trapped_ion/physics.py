...
def collect_mode_snapshots(
    ...
    Iterates over ManipulationTraps in ``architecture.qccd_graph``,
    takes a snapshot of each trap's mode structure, and maps every
    qubit in that trap to the snapshot.

    Parameters
    ----------
    architecture
        The trapped-ion architecture with a ``qccd_graph`` attribute.
    qubit_ids : Dict[int, float]
    ...
    try:
        ...
        from qectostim.experiments.hardware_simulation.trapped_ion.architecture import (
            ManipulationTrap as _MT,
        )
        if hasattr(architecture, 'qccd_graph'):
            for _nid, node in architecture.qccd_graph.nodes.items():
                if isinstance(node, _MT) and node.mode_structure is not None:
                    snap = node.mode_structure.snapshot()
                    for ion in node.ions:
                        if ion.idx in qubit_ids:
                            result[ion.idx] = snap
    except Exception:
